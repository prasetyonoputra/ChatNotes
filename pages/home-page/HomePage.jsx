import React, { useEffect, useState } from "react";
import { ScrollView, Text, View, StyleSheet, Alert } from "react-native";
import BottomBarComponent from "../../components/bottom-bar/BottomBarComponent";
import SearchNoteComponent from "../../components/search-notes/SearchNoteComponent";
import SidebarComponent from "../../components/sidebar-component/SidebarComponent";

function HomePage() {
    const [openSidebar, setOpenSidebar] = useState(false);

    return (
        <View style={styles.container}>
            <View style={styles.searchBar}>
                <SearchNoteComponent />
            </View>

            <ScrollView>
                <Text>Main</Text>
            </ScrollView>

            <View style={styles.bottomBar}>
                <BottomBarComponent setOpenSidebar={setOpenSidebar} openSidebar={openSidebar} />
            </View>

            {openSidebar && <SidebarComponent setOpenSidebar={setOpenSidebar} openSidebar={openSidebar} />}
        </View>
    );
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
    },
    searchBar: {
        position: "absolute",
        top: 0,
        left: 0,
        right: 0,
        zIndex: 1,
    },
    bottomBar: {
        position: "absolute",
        bottom: 0,
        left: 0,
        right: 0,
    },
});

export default HomePage;
